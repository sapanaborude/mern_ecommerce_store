...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("https://mern-ecommerce-store-37ut.onrender.com/api/v1/category/create-category", {
        catName,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://mern-ecommerce-store-37ut.onrender.com/api/v1/category/update-category/${catSelected._id}`,
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://mern-ecommerce-store-37ut.onrender.com/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
